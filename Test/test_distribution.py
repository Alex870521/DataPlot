...
from DataPlot.plot_templates.distribution import plot_dist, plot_dist2, plot_dist_with_STD, plot_dist_cp, plot_dist_fRH
...
Ext_amb_df = concat([df[['Extinction', 'State']], PESD_inter], axis=1)
...
Ext_amb_dis, Ext_amb_dis_std = get_statistic(Ext_amb_df.dropna().groupby('State'))
...
PSD_amb_dis, PSD_amb_dis_std = get_statistic(PSD_amb_df.dropna().groupby('State'))
