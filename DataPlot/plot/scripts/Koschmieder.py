...
def koschmieder(df: pd.DataFrame,
                yy: Literal['Vis_Naked', 'Vis_LPV'],
                function: Literal['log', 'reciprocal'] = 'log',):

    _df1 = df[['Extinction', 'ExtinctionByGas', f'{yy}']].dropna().copy()
    _df2 = df[['total_ext_dry', 'ExtinctionByGas', f'{yy}']].dropna().copy()
